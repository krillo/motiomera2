...
var Account = {
  launch: function(element, url) {
    ...
    var link = clone.down("a");
    var progress = clone.down(".loading");

    var wrapper = new Element("div").insert(clone);
    var rocket = new Element("div").setStyle({
      position: "absolute",
      left:     offsets.left + "px",
      top:      offsets.top + "px",
      width:    element.getStyle("width"),
      height:   element.getStyle("height"),
      zIndex:   1000
    }).insert(wrapper).addClassName("rocket");

    var cloneDimensions = element.getDimensions(), viewportDimensions = document.viewport.getDimensions();
    var left = viewportDimensions.width / 2 - cloneDimensions.width / 2;
    var top = viewportDimensions.height / 2 - cloneDimensions.height / 2;

    element.down("a").setStyle({ visibility: "hidden" });
    $(document.body).insert(rocket).addClassName("launching");

    $(document.body).down(".container").fade({ duration: 0.25 });

    if ($("launchbar")) {
      $("launchbar").hide();
      (function() { $("launchbar").show() }).delay(0.75);
      Launchbar.selectCurrent(element.readAttribute("data-product"), element.readAttribute("data-subdomain"));
    }

    wrapper.transition({ animate: !Prototype.Browser.IE, duration: 0.25 }, function() {
      clone.addClassName("busy");
    }).after(function() {
      ...
      function progressBar() {
        ...
        clone.down('.loading').setStyle({ backgroundPosition: leftPos  + "px -850px" });
...
document.observe("dom:loaded", function() {
  if ($(document.body).hasClassName("users")) {
    window.dragObserver = new DragObserver();
    window.reorderObserver = new ReorderObserver();

    var justReorderedElement;

    function isIgnorable(event) {
      return event.button > 1 || event.ctrlKey || event.metaKey;
    }

    function clickedOnIconOrName(event) {
      return event.findElement("span.logo") || event.findElement("div.name div")
    }

    document.observe("drag:requested", function(event) {
      if (isIgnorable(event.memo.mouseEvent)) {
        event.stop();
      }

      var element = event.findElement("div.products div.account");
      if (element && element.hasClassName("active_menu")) {
        event.stop();
      }
    });

    document.observe("order:stopped", function(event) {
      var element = event.findElement("div.products div.account");
      if (element) {
        justReorderedElement = element;
        (function() { justReorderedElement = false }).defer();
      }
    });
    ...
    if (navigator.userAgent.match(/iPad/)) {
      document.observe("touchstart", function(event) {
        ...
        if (element) {
          event.stop();
          Account.launch(element, element.down("a").readAttribute("href"));
        }
      });
    } else {
      document.observe("click", function(event) {
        var element = event.findElement("div.products div.account");
        if (element) {
          if (isIgnorable(event) || event.stopped || element == justReorderedElement) return;
          event.stop();
          if (!clickedOnIconOrName(event) || element.hasClassName("reordering") || element.hasClassName("active_menu")) return;
          Account.launch(element, element.down("a").readAttribute("href"));
        }
      });
    }
...
document.observe("order:changed", function(event) {
...
document.observe("dom:loaded", function() {
  if ($(document.body).hasClassName("users")) {
    if (navigator.userAgent.match(/iPad/)) {
      $("users").select("div.account > div.menu_target").invoke("remove");
      return;
    }

    new MenuObserver("users");
    new HoverObserver("users");

    $("users").observe("menu:prepared", function(event) {
      var element = event.element().down(".menu_content");
      element.addClassName("delay_deactivate");

      (function() {
        element.removeClassName("delay_deactivate");
      }).delay(0.5)
    });

    $("users").observe("click", function(event) {
      if (event.findElement(".menu_content.delay_deactivate")) return;

      var element = event.element();
      if (element.hasClassName("jump") || element.hasClassName("menu_content"))
        element.fire('menu:deactivate');
    });

    $("users").observe("hover:deactivated", function(event) {
      var element = event.element().down(".menu_content");
      if (element) element.fire('menu:deactivate');
    });

    $("users").observe("menu:selected", function(event) {
      var element = event.findElement("div.products div.account");
      if (element && event.memo.action == "link")
        Account.launch(element, event.memo.value);
    });

    $("users").observe("menu:activated", function(event) {
      var element;

      if (element = event.findElement(".accounts")) {
        element.addClassName("has_active_menu");
        element.up(".product").addClassName("has_active_menu");
      }
    });

    $("users").observe("menu:deactivated", function(event) {
      var element;

      if (element = event.findElement(".accounts")) {
        element.removeClassName("has_active_menu");
        element.up(".product").removeClassName("has_active_menu");
      }
    });
  }
});
...
Element.addMethods({
  setState: function(element, stateName) {
    element = $(element);
    element.writeAttribute("data-state", stateName);

    element.addClassName("bogus").removeClassName("bogus");
...
var Login = {
  loginLoading: function() {
    $("login_content").setState("loading");
    $("login_content").removeClassName("wide_error");
  }
};

document.observe("dom:loaded", function() {
  if ($(document.body).hasClassName("login")) {
  ...
  if ($("progressBar")) {
    ...
    function progressBar() {
      ...
      $('progressBar').setStyle({ backgroundPosition: leftPos  + 'px -850px' });
      setTimeout(progressBar, 100);
    };
    progressBar();
  }
